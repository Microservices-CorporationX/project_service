...
class TaskControllerTest {
    ...
    void updateTask_ShouldThrowIllegalArgumentException_WhenTaskNotFound() {
        ...
        when(taskService.updateTask(1L, updatedTaskDTO, 1L))
            .thenThrow(new IllegalArgumentException("Задача с таким ID не найдена"));
    ...
    void getTaskById_ShouldThrowIllegalArgumentException_WhenTaskNotFound() {
        when(taskService.getTaskById(1L, 1L))
            .thenThrow(new IllegalArgumentException("Задача с таким ID не найдена"));
    ...
    void getFilteredTasks_ShouldReturnFilteredTasks() {
        ...
        when(taskService.getFilteredTasks(taskFilterDTO, 1L))
    ...
    void getFilteredTasks_ShouldThrowValidationException_WhenFilterIsNull() {
        when(taskService.getFilteredTasks(null, 1L))
            .thenThrow(new ValidationException("Фильтр не может быть null"));
